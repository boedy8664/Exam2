...
int feature[1000];
int feature_ring[1000];
int feature_slope[1000];
int feature_vertical[1000];
  ...
  while (true) {
    ...
    if (gesture_index < label_num) {
      ...
      if (gesture_index == 0) {
        uLCD.cls();
        uLCD.printf("\ngestureID = %d\n", gesture_index);
        feature_ring[a] = feature[a];
        //angle = 30;
      }
      else if (gesture_index == 1) {
        uLCD.cls();
        uLCD.printf("\ngestureID = %d\n", gesture_index);
        feature_slope[a] = feature[a];
        //angle = 35;
      }
      else if (gesture_index == 2) {
        uLCD.cls();
        uLCD.printf("\ngestureID = %d\n", gesture_index);
        feature_vertical[a] = feature[a];
        //angle = 40;
      }
      co++;
...
void tilt(Arguments *in, Reply *out) {
  for (int i = 0; i < 200; i++) {
    printf("\nRing\n");
    if()
    printf("Ring, %d", feature_ring[i]);
  }
  for (int i = 0; i < 200; i++) {
    printf("\nSlope\n");
    printf("%d", feature_ring[i]);
  }
  for (int i = 0; i < 200; i++) {
    printf("\nVertical\n");
    printf("Ring, %d", feature_ring[i]);
